...
import { creaFatturaAttiva } from '@/app/impresa/fatture/actions'
...
export default function NuovaFatturaFormUfficio({ clienti, commesse }: { clienti: Cliente[]; commesse: Commessa[] }) {
  ...
  const imponibile = righe.reduce((acc, r) => acc + (parseFloat(r.quantita) || 0) * euroToCents(r.prezzoUnitario), 0)
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        ...
        router.push(`/ufficio/fatture/${id}`)
      } catch (err: unknown) {
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5 space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <input type="text" value={numero} onChange={e => setNumero(e.target.value)} placeholder="es. 001"
              className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:border-teal-500 focus:outline-none" required />
          </div>
          <div>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <select value={aliquotaIva} onChange={e => setAliquotaIva(e.target.value)}
              className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:border-teal-500 focus:outline-none">
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5 space-y-4">
        <div className="flex items-center justify-between">
          ...
          <button type="button" onClick={aggiungiRiga} className="text-sm text-teal-600 hover:text-teal-800 font-medium">+ Aggiungi voce</button>
        </div>
      ...
      <div className="flex gap-3">
        <Link href="/ufficio/fatture" className="flex-1 rounded-xl border border-gray-300 px-4 py-2 text-center text-sm font-medium text-gray-700 hover:bg-gray-50">Annulla</Link>
