...
import {
  ...
  eliminaPromemoria, aggiornaPromemoria, toggleImportante,
  getOperaiDropdown, getClientiDropdown, getCommesseDropdown, getPromemoriaScaduti,
} from './actions'
import { AssistentePromemoria } from '@/components/ai/AssistentePromemoria'
import { PromemoriaScaduti } from '@/components/ai/PromemoriaScaduti'

type Filtro = 'tutti' | 'oggi' | 'futuri' | 'completati'
type OperaioDropdown = { id: string; nome: string }
type Promemoria = {
  id: string; titolo: string; tipo: string; descrizione?: string | null; luogo?: string | null
...
const FORM_VUOTO = {
  ...
  dataOra: '', assegnatoAOperaioId: '', perImpresa: true, importante: false, priorita: 'normale',
}
...
export default function PromemoriaPage() {
  ...
  useEffect(() => {
    ...
    getClientiDropdown().then(setClienti).catch(console.error)
    getCommesseDropdown().then(setCommesse).catch(console.error)
  }, [])
  ...
  useEffect(() => {
    getPromemoriaScaduti()
      .then(data => setScaduti(data as Promemoria[]))
      .catch(console.error)
  }, [])

  function reload() {
    getPromemoriaFiltro(filtro).then(d => setItems(d as Promemoria[])).catch(console.error)
    getPromemoriaScaduti().then(d => setScaduti(d as Promemoria[])).catch(console.error)
  }
  ...
  function apraModifica(item: Promemoria) {
    ...
    setForm({
      ...
      priorita:            item.priorita ?? 'normale',
    })
    setPannello(item.id)
  }
  ...
  function handleSalva(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        ...
        if (pannello === 'new') { await creaPromemoria(payload) }
        else if (pannello)       { await aggiornaPromemoria(pannello, payload) }
        setPannello(null)
        reload()
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-0">
      ...
      {/* Promemoria scaduti da verificare */}
      <PromemoriaScaduti scaduti={scaduti} onAggiornato={reload} accentColor="blue" />

      {/* Assistente AI */}
      <AssistentePromemoria
        operai={operai}
        clienti={clienti}
        commesse={commesse}
        onSalvato={reload}
      ...
      <div className="flex gap-5">
        ...
        <div className="w-56 flex flex-col gap-1.5 shrink-0">
            ...
            return (
              <button
                ...
                className={`flex items-center gap-3 px-3.5 py-2.5 rounded-xl transition-all text-sm font-medium text-left ${
                  active ? 'bg-blue-50 text-blue-700 border border-blue-100 shadow-sm' : 'text-slate-600 hover:bg-slate-100 border border-transparent'
                }`}
              >
          ...
          <div className="mt-6 rounded-xl border border-gray-200 bg-white p-4 space-y-2.5">
            ...
            {scaduti.length > 0 && (
              <div className="flex justify-between text-sm">
                <span className="text-orange-600">Scaduti</span>
                <span className="font-bold text-orange-700">{scaduti.length}</span>
              </div>
            )}
          </div>
        </div>
          ...
          <div className="flex-1 overflow-y-auto">
            {loading ? (
              ...
              <div className="flex flex-col items-center justify-center h-full text-center py-16 px-6">
                ...
                <p className="text-xs text-slate-400 mb-4">Crea il primo con il pulsante qui sopra o usa l'assistente AI.</p>
                <button onClick={apraNuovo} className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl px-4 py-2 text-sm font-semibold transition-colors">
              ...
              <div className="divide-y divide-gray-100">
                {items.map(item => {
                  ...
                  const priorita   = item.priorita || 'normale'
                  return (
                    <div
                      key={item.id}
                      className={`flex items-start gap-3 px-4 py-3.5 transition-colors ${isEditing ? 'bg-blue-50/60' : 'hover:bg-gray-50'}`}
                    >
                      <button onClick={() => handleCompleta(item.id, completato)} className="mt-0.5 shrink-0 transition-transform hover:scale-110">
                        {completato
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          {!completato && priorita !== 'normale' && (
                            <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full border ${COLOR_PRIORITA[priorita]}`}>
                        ...
                        <div className="flex flex-wrap items-center gap-x-3 gap-y-1 mt-1">
                          <span className="flex items-center gap-1 text-[11px] font-medium text-blue-600 bg-blue-50 px-2 py-0.5 rounded-md border border-blue-100">
                            <Clock size={10} />
                            {new Date(item.dataOra).toLocaleString('it-IT', { day: '2-digit', month: 'short', hour: '2-digit', minute: '2-digit', timeZone: 'Europe/Rome' })}
                          </span>
                          {item.luogo && <span className="flex items-center gap-1 text-[11px] text-slate-500"><MapPin size={10} />{item.luogo}</span>}
                          {item.operaio && <span className="flex items-center gap-1 text-[11px] text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-md"><User size={10} />{item.operaio.nome}</span>}
                          {item.cliente && <span className="text-[11px] text-violet-600 bg-violet-50 px-2 py-0.5 rounded-md">{item.cliente.nome}</span>}
                          {item.commessa && <span className="text-[11px] text-gray-500 bg-gray-100 px-2 py-0.5 rounded-md">{item.commessa.nome}</span>}
                        </div>
                      </div>

                      <div className="flex items-center gap-0.5 shrink-0">
                        <button onClick={() => handleImportante(item.id, item.importante)} title={item.importante ? 'Rimuovi da importanti' : 'Segna come importante'} className="p-1.5 rounded-lg hover:bg-amber-50 transition-colors">
                          <Star size={15} className={item.importante ? 'fill-amber-400 text-amber-400' : 'text-slate-300 hover:text-amber-400'} />
                        </button>
                        <button onClick={() => apraModifica(item)} title="Modifica" className={`p-1.5 rounded-lg transition-colors ${isEditing ? 'text-blue-600 bg-blue-100' : 'text-slate-400 hover:text-blue-600 hover:bg-blue-50'}`}>
                          <Edit2 size={15} />
                        </button>
                        <button onClick={() => handleElimina(item.id)} title="Elimina" className="p-1.5 rounded-lg text-slate-300 hover:text-red-600 hover:bg-red-50 transition-colors">
                          <Trash2 size={15} />
                        </button>
                      </div>
                    </div>
          ...
          <div className="w-80 bg-white rounded-2xl border border-gray-200 shadow-sm p-5 overflow-y-auto shrink-0">
            ...
            <form onSubmit={handleSalva} className="space-y-4">
              <div>
                ...
                <textarea value={form.titolo} onChange={e => setForm(f => ({ ...f, titolo: e.target.value }))} rows={2} required placeholder="Es. Sopralluogo Via Roma 15…"
              ...
              <div>
                ...
                <input type="datetime-local" value={form.dataOra} onChange={e => setForm(f => ({ ...f, dataOra: e.target.value }))} required
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">Priorità</label>
              ...
              <div>
                ...
                <input type="text" value={form.luogo} onChange={e => setForm(f => ({ ...f, luogo: e.target.value }))} placeholder="Es. Via Roma 15, Roma"
                  className="w-full text-sm text-slate-700 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white" />
              </div>

              <div>
                <label className="text-[11px] font-bold text-slate-400 uppercase tracking-wider mb-1 block">Assegna a operaio</label>
                <select value={form.assegnatoAOperaioId} onChange={e => setForm(f => ({ ...f, assegnatoAOperaioId: e.target.value }))}
                  className="w-full text-sm text-slate-700 bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 outline-none focus:ring-2 focus:ring-blue-500/20 focus:bg-white">
              ...
              <div>
                ...
                <textarea value={form.descrizione} onChange={e => setForm(f => ({ ...f, descrizione: e.target.value }))} rows={3} placeholder="Aggiungi dettagli o istruzioni…"
              ...
              <label className="flex items-center gap-2.5 cursor-pointer group">
                <div onClick={() => setForm(f => ({ ...f, importante: !f.importante }))}
              ...
              <div className="pt-3 border-t border-slate-100">
                <button type="submit" disabled={pending || !form.titolo.trim() || !form.dataOra}
