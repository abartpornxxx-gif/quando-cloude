...
import { creaFatturaPassiva } from '@/app/impresa/fatture-passive/actions'
...
export default function NuovaFatturaPassivaFormUfficio({
  fornitori, commesse, ordini,
}: {
  fornitori: Fornitore[]
  commesse: Commessa[]
  ordini: Ordine[]
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    startTransition(async () => {
      try {
        ...
        router.push(`/ufficio/fatture-passive/${id}`)
      } catch (err: unknown) {
      ...
      <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5 space-y-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <input type="text" value={numero} onChange={e => setNumero(e.target.value)} placeholder="Numero fattura del fornitore"
              className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:border-teal-500 focus:outline-none" />
          </div>
        </div>
        ...
        <div>
          ...
          <input type="number" min="0.01" step="0.01" value={importo} onChange={e => setImporto(e.target.value)} placeholder="0.00"
            className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:border-teal-500 focus:outline-none" required />
        </div>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          <div>
          ...
          <div>
            ...
            <select value={ordineId} onChange={e => setOrdineId(e.target.value)}
              className="w-full rounded-xl border border-gray-300 px-3 py-2 text-sm focus:border-teal-500 focus:outline-none">
              <option value="">— Nessun ordine —</option>
              {ordini.map(o => (
                <option key={o.id} value={o.id}>{o.fornitore?.nome ?? 'Ordine'} — {new Date(o.createdAt).toLocaleDateString('it-IT')}</option>
              ))}
            </select>
          </div>
        </div>
