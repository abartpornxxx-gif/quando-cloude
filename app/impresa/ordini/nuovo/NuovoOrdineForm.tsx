...
export default function NuovoOrdineForm({
  fornitori,
  commesse,
  materiali,
}: {
  ...
  function selezionaMateriale(idx: number, matId: string) {
    ...
    if (mat) {
      setRighe(prev => prev.map((r, i) =>
        i === idx
          ? { ...r, materialeId: matId, descrizione: mat.descrizione, prezzoUnitario: centsToInput(mat.prezzo) }
          : r
      ))
    } else {
          ...
          {righe.map((r, idx) => (
            <div key={idx} className="rounded-lg border border-gray-100 bg-gray-50 p-3 space-y-2">
              ...
              <div>
                ...
                <select
                  value={r.materialeId}
                  onChange={e => selezionaMateriale(idx, e.target.value)}
                ...
                >
                  ...
                  {materiali.map(m => (
                    <option key={m.id} value={m.id}>
                      {m.codice ? `[${m.codice}] ` : ''}{m.descrizione} — {(m.prezzo / 100).toFixed(2)} €/{m.unita ?? 'pz'}
                    </option>
                  ))}
                </select>
              </div>
