...
import { eliminaMagazziniere, ripristinaPasswordMagazziniere } from './actions'
import { ResetPasswordButton } from '@/components/ResetPasswordButton'

export default async function MagazzinieriPage() {
  await requireImpresa()
  const magazzinieri = await prisma.magazziniere.findMany({ 
    orderBy: { nome: 'asc' },
    select: {
      id: true,
      nome: true,
        ...
        <div className="rounded-2xl border border-gray-200 bg-white shadow-sm overflow-hidden">
          <div className="divide-y divide-gray-100">
            {magazzinieri.map(m => {
              ...
              return (
                <div key={m.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-5 py-4 hover:bg-gray-50/70 transition-colors group">
                  <div className="flex items-center gap-4 flex-1 min-w-0">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <p className="text-sm font-semibold text-gray-900 group-hover:text-blue-700 transition-colors truncate">{m.nome}</p>
                        
                        {/* Stato Accesso/Password Badges */}
                        {m.primoAccesso && m.passwordResetRichiesto ? (
                  ...
                  <div className="flex items-center gap-4 sm:gap-3 pl-13 sm:pl-0 shrink-0">
                    <ResetPasswordButton id={m.id} nome={m.nome} action={ripristinaPasswordMagazziniere} />
