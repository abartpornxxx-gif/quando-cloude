...
import RegistraIncassoForm from './RegistraIncassoForm'
import { eliminaFatturaAttiva, segnaScaduta } from '../actions'
...
export default async function FatturaAttivaPage({ params }: Props) {
  ...
  return (
    <div className="p-4 max-w-3xl mx-auto space-y-5">
      <div className="flex items-center gap-3">
        <Link href="/impresa/fatture" className="text-blue-600 hover:text-blue-800 text-sm">‹ Fatture</Link>
        <h1 className="text-xl font-bold flex-1">
          Fattura n. {fattura.numero}/{fattura.anno}
        </h1>
        <span className={`text-xs rounded-full px-3 py-1 font-semibold ${BADGE[derivedStatus]}`}>
          {LABEL[derivedStatus]}
        </span>
      </div>
      ...
      <div className="flex flex-wrap gap-3">
        <a
          href={`/api/fatture/${fattura.id}/xml`}
          target="_blank"
        ...
        {derivedStatus === 'da_incassare' && fattura.dataScadenza && new Date(fattura.dataScadenza) < new Date() && (
          <form action={segnaScaduta.bind(null, fattura.id)}>
        ...
        {(derivedStatus === 'da_incassare' || derivedStatus === 'scaduta') && (
