  ...
  return (
    <div className="mx-auto max-w-xl">
      <div className="mb-6 flex items-center gap-3">
        ...
        <h1 className="text-2xl font-bold text-gray-900">Nuovo mezzo</h1>
      </div>
      <MezzoForm action={salvaMezzo} />
    </div>
  )
}

export function MezzoForm({ action, dv }: { action: (fd: FormData) => Promise<void>; dv?: Record<string, string> }) {
  return (
    <form action={action} className="space-y-4 rounded-xl border border-gray-200 bg-white p-6 shadow-sm">
      ...
      <Field label="Nome mezzo *" name="nome" required defaultValue={dv?.nome} />
      <Field label="Targa" name="targa" defaultValue={dv?.targa} />
      <div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        <Field label="Scad. bollo" name="scadenzaBollo" type="date" defaultValue={dv?.scadenzaBollo} />
        <Field label="Scad. revisione" name="scadenzaRevisione" type="date" defaultValue={dv?.scadenzaRevisione} />
        <Field label="Scad. assicurazione" name="scadenzaAssicurazione" type="date" defaultValue={dv?.scadenzaAssicurazione} />
      </div>
      <div>
      ...
      <div className="flex gap-3 pt-2">
        ...
        <Link href="/impresa/mezzi" className="rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50">Annulla</Link>
      </div>
    </form>
  )
}
