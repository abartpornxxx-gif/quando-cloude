...
import { inviaMsgMagazziniere, getMessaggiMagazziniere } from './actions'
...
const COLORE: Record<string, string> = {
  ...
  magazziniere: 'bg-yellow-100 text-gray-800 rounded-tr-none border border-yellow-200',
}

export default function ChatMagazziniere({ giornataId, messaggi: messaggiIniziali }: Props) {
  ...
  useEffect(() => {
    const poll = setInterval(async () => {
      try {
        const nuovi = await getMessaggiMagazziniere(giornataId)
  ...
  function invia() {
    ...
    startTransition(async () => {
      try {
        await inviaMsgMagazziniere(giornataId, testo)
        setTesto('')
        const nuovi = await getMessaggiMagazziniere(giornataId)
        setMessaggi(nuovi as Messaggio[])
      } catch (err: unknown) {
  ...
  return (
    <div className="flex flex-col" style={{ height: 'calc(100vh - 110px)' }}>
      <div className="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        {messaggi.map(m => (
          <div key={m.id} className={`flex ${m.ruolo === 'magazziniere' ? 'justify-end' : 'justify-start'}`}>
      ...
      <div className="border-t bg-white p-3 flex gap-2 items-end">
        <textarea
          ...
          placeholder="Scrivi all'operaio…"
        ...
        <button
          ...
          className="bg-yellow-600 text-white rounded-xl px-4 py-2 text-sm font-semibold disabled:opacity-50"
        >
          {pending ? '…' : 'Invia'}
        </button>
      </div>
