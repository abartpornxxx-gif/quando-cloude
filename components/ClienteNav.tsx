...
const MACRO: MacroEntry[] = [
  {
    ...
    dot: 'bg-amber-400',
    prefixes: ['/cliente/lavori', '/cliente/manutenzioni'],
    items: [
      { label: 'I miei lavori', href: '/cliente/lavori', desc: 'I miei cantieri e lavori' },
      { label: 'Manutenzioni', href: '/cliente/manutenzioni', desc: 'Interventi programmati' },
    ],
  },
  {
    ...
    dot: 'bg-emerald-500',
    prefixes: ['/cliente/pagamenti', '/cliente/documenti'],
    items: [
      { label: 'Pagamenti', href: '/cliente/pagamenti', desc: 'Fatture e pagamenti' },
      { label: 'Documenti', href: '/cliente/documenti', desc: 'Tutti i documenti' },
    ],
  },
  {
    label: 'Servizi',
    ...
    dot: 'bg-cyan-500',
    prefixes: ['/cliente/servizi'],
    items: [
      { label: 'Servizi', href: '/cliente/servizi', desc: 'Catalogo servizi' },
    ],
  },
]
...
export function ClienteNav() {
  ...
  return (
    <div className="border-t border-violet-700/60 bg-violet-850 bg-violet-800">
        ...
        <div className="hidden md:flex items-stretch">
          ...
          <Link
            ...
            className={`shrink-0 flex items-center gap-1.5 px-4 py-2.5 text-sm font-medium border-b-2 transition-colors ${
              isDashboard
                ? 'border-violet-400 text-violet-250 text-violet-200'
                : 'border-transparent text-violet-100 hover:text-white'
            }`}
          >
            <LayoutDashboard size={13} className={isDashboard ? 'text-violet-200' : 'text-violet-300'} />
            Dashboard
          </Link>
              ...
              if (activeMacro) {
                ...
                return (
                  <>
                    ...
                    <span className="text-sm text-violet-100 font-medium">{activeMacro.label}</span>
                  </>
                )
              }
              return <span className="text-sm text-violet-100 font-medium">Dashboard</span>
            })()}
          </div>
          <button
            onClick={() => setMenuAperto(v => !v)}
            className="rounded-lg p-2 text-violet-100 hover:bg-violet-700 hover:text-white transition-colors"
              ...
              <nav className="space-y-5 overflow-y-auto max-h-[70vh]">
                <Link
                  ...
                  className={`flex items-center gap-2.5 px-3 py-2 rounded-xl text-sm font-semibold transition-all ${
                    isDashboard ? 'bg-violet-50 text-violet-700' : 'text-gray-700 hover:bg-gray-50'
                  }`}
                >
                          ...
                          return (
                            <Link
                              ...
                              className={`block px-3 py-2 rounded-xl text-sm font-medium transition-all ${
                                itemActive ? 'bg-violet-50 text-violet-700 font-semibold' : 'text-gray-600 hover:bg-gray-50'
                              }`}
                            >
                              {item.label}
                            </Link>
